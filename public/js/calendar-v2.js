...
const statusConfig = {
  ...
  arrived: { color: '#f59e0b', bg: 'linear-gradient(135deg, #1f1f1f 0%, #2d2d2d 100%)', textColor: '#fbbf24', name: 'Arrived', badge: '#a16207' },
  paid: { color: '#10b981', bg: 'linear-gradient(135deg, #0f2818 0%, #1a3a2a 100%)', textColor: '#6ee7b7', name: 'Paid', badge: '#065f46' },
  pending: { color: '#a855f7', bg: 'linear-gradient(135deg, #2d1b69 0%, #3d2463 100%)', textColor: '#d8b4fe', name: 'Pending', badge: '#581c87' },
  cancelled: { color: '#ef4444', bg: 'linear-gradient(135deg, #3d0e0e 0%, #4a1515 100%)', textColor: '#f87171', name: 'Cancelled', badge: '#7f1d1d' }
};
...
function renderDayView(date, events) {
  ...
  for (let h = WORK_START; h <= WORK_END; h++) {
    timeCol.innerHTML += `<div style="height:${PX_PER_HOUR}px;line-height:${PX_PER_HOUR}px;padding-left:8px;font-weight:600;color:#94a3b8;font-size:0.85rem;">${String(h).padStart(2, '0')}:00</div>`;
  }
  ...
  (DOCTORS || []).filter(d => parseInt(d.show_in_calendar) === 1).forEach(d => {
    ...
    if (calendarGrid) {
      ...
      calendarGrid.style.height = `${(WORK_END - WORK_START + 1) * PX_PER_HOUR}px`;
    ...
    // Show calendar background for non-working times; draw working hours as brown striped bar
    // (non-working time will inherit column background `#0f172a` set above)
    if (todayWorkHours && parseInt(todayWorkHours.is_available) === 1) {
      ...
      // Always fill brown area to 18:00 (720px) minimum
      const maxVisiblePx = (WORK_END - WORK_START) * PX_PER_HOUR; // 720px
      let endPx = maxVisiblePx; // Always fill to 18:00
      if (endPx > startPx + 2) {
        const workEl = document.createElement('div');
        workEl.style.position = 'absolute';
        workEl.style.left = '8px';
        workEl.style.right = '8px';
        workEl.style.top = `${startPx}px`;
        workEl.style.height = `${Math.max(4, endPx - startPx)}px`;
        workEl.style.background = '#b07a50'; // darker brown working area
        workEl.style.border = '1px solid rgba(0,0,0,0.18)';
        workEl.style.borderRadius = '6px';
  ...
  (events || []).forEach(ev => {
    ...
    el.innerHTML = `
      <div style="font-weight:700;color:${cfg.textColor};font-size:.95rem;line-height:1.15;display:flex;align-items:center;gap:8px;">
        <span style="opacity:.85;font-size:.85rem;">${ev.start_time}–${ev.end_time}</span>
        <span style="margin-left:6px;color:#cbd5e1;font-weight:600;min-width:0;">${esc(ev.patient_name || '')}</span>
        ${ev.phone ? `<span style="margin-left:8px;background:#0ea5a5;color:#012; padding:0.12rem 0.5rem;border-radius:6px;font-size:0.78rem;font-weight:700;flex-shrink:0;">${esc(ev.phone)}</span>` : ''}
      </div>
      <div style="margin-top:6px;display:flex;align-items:center;gap:8px;color:#c7d2fe;font-weight:600;">
  ...
  for (let i = 0; i < 7; i++) {
    ...
    (DOCTORS || []).filter(doc => parseInt(doc.show_in_calendar) === 1).forEach(doc => {
      ...
      if (whData && parseInt(whData.is_available) === 1) {
        ...
        // Always fill brown area to 18:00 (720px) minimum
        const maxVisiblePx = (WORK_END - WORK_START) * PX_PER_HOUR; // 720px
        let endTop = maxVisiblePx; // Always fill to 18:00
        if (endTop > startTop + 2) {
          ...
          workDiv.style.background = '#b07a50';
          workDiv.style.border = '1px solid rgba(0,0,0,0.12)';
          workDiv.style.boxShadow = 'inset 0 1px 0 rgba(0,0,0,0.12)';
          workDiv.style.borderRadius = '6px';
    ...
    (events || []).filter(ev => ev.date === ds).forEach(ev => {
      ...
      el.innerHTML = `
        <div style="font-weight:700;color:${cfg.textColor};display:flex;align-items:center;gap:6px;">
          <span style="opacity:.85;font-size:.9rem;">${ev.start_time}–${ev.end_time}</span>
          <span style="margin-left:6px;color:#cbd5e1;font-weight:600;min-width:0;">${esc(ev.patient_name || '')}</span>
          ${ev.phone ? `<span style="margin-left:8px;background:#0ea5a5;color:#012;padding:0.08rem 0.45rem;border-radius:6px;font-size:0.75rem;font-weight:700;flex-shrink:0;">${esc(ev.phone)}</span>` : ''}
        </div>
        <div style="margin-top:4px;display:flex;align-items:center;gap:6px;color:#c7d2fe;font-size:0.78rem;font-weight:600;">
